...
async function runBuildWithFeedback(runId, runDir, mode, io, logTokens) {
  ...
  while (true) {
    const result = mode === "auto"
      ? await runDivisionAuto("departments/build/runner.js", ["--run-id", runId], (sig) => handleBuildSignal(runDir, sig, io, logTokens), runDir)
  ...
  while (currentId) {
    ...
    if (skip) {
      ...
      if (node.feedbackLoop) {
        ...
        result = await runBuildWithFeedback(runId, runDir, mode, io, logTokens);
      } else {
