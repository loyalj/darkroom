...
async function runBuildWithFeedback(runId, runDir, mode, io, logTokens, managerRoleId, orgProfile) {
  ...
  while (true) {
    ...
    let feedback;
    if (mode === "auto") {
      feedback = await makeVerificationFeedbackDecision(runDir, neededData.failures ?? [], io, logTokens, managerRoleId, orgProfile);
      if (feedback === null) {
        console.log(`\n  ${A.cyan("●")}  Verification failures accepted — proceeding.\n`);
        return { code: 0, stderr: "" };
      }
    } else {
      feedback = await io.turn('Your feedback (or "accept" to proceed with failures): ');
      if (/^(skip|accept|proceed)$/i.test(feedback.trim())) {
        console.log(`  Verification failures accepted — proceeding.\n`);
        return { code: 0, stderr: "" };
      }
      if (!feedback.trim()) {
...
async function escalate(runDir, reason, context, io) {
  ...
  const raw = await io.turn("  Your choice: ", { options, escalation: true });
...
- To request copy revision: {"decision":"reject","confidence":"high"|"medium"|"low","feedback":"...specific feedback for the copywriter...","reasoning":"..."}
- To request a code fix:    {"decision":"code-fix","confidence":"high"|"medium"|"low","feedback":"...specific instructions for the code fix agent...","reasoning":"..."}
- To escalate to human:     {"decision":"escalate","reasoning":"...why you cannot decide confidently..."}

Use "code-fix" when the copy problem cannot be resolved by rewriting copy — e.g. user-facing strings are missing because the code never generates them, the UI element referenced in the copy doesn't exist, or the copy faithfully reflects a code bug. The fix agent will patch the source code; copy will be regenerated afterward.
Use "reject" only when the copy itself is wrong (voice, tone, wording) but the underlying code is correct.
Base your decision on the operator's copy voice preferences and quality bar from the brain.`,
  ...
  if (result.decision === "escalate" || result.confidence === "low") {
    ...
    const answer = await escalate(runDir, "Low confidence on copy review", { at: "Build Phase 4 — Copy Review", detail: result.reasoning ?? "The orchestrator could not determine whether this copy matches your voice.", options: ["approve", "reject: <feedback>", "fix: <code feedback>", "abort"] }, io);
    if (/^abort/i.test(answer)) process.exit(1);
    const humanApproved = /^approve/i.test(answer);
    const humanCodeFix = /^fix:/i.test(answer);
    const humanFeedback = answer.replace(/^(reject|fix):\s*/i, "").trim();
    const humanDecision = humanApproved ? "approve" : humanCodeFix ? "code-fix" : "reject";
    writeDecision(runDir, { decisionPoint: "copy-review", evidence: copyContent.slice(0, 200), brainContext: "(escalated)", decision: humanDecision, reasoning: "Human override after escalation.", humanOverride: true });
    if (humanCodeFix) return `__CODE_FIX__: ${humanFeedback || "Fix the code so the copy can be accurately extracted."}`;
  ...
  const decisionLabel = result.decision === "code-fix" ? A.yellow("code-fix") : approved ? A.green("approve") : A.yellow("reject");
  console.log(`\n  ${A.cyan("●")}  Auto decision: copy-review → ${decisionLabel}  ${A.dim("(" + confidenceColor(result.confidence ?? "?") + " confidence)")}`);
  if (result.reasoning) console.log(`  ${A.dim("↳ " + result.reasoning.split(/[.\n]/)[0].trim())}\n`);
  if (result.decision === "code-fix") return `__CODE_FIX__: ${result.feedback ?? "Fix the code so the copy can be accurately extracted."}`;
  return approved ? "yes" : (result.feedback ?? "Please revise the copy.");
}

async function makeVerificationFeedbackDecision(runDir, failures, io, logTokens, managerRoleId, orgProfile) {
  const brainContent    = org.getBrainForRole(managerRoleId, orgProfile);
  const runBrainPath    = path.join(runDir, "run-brain.md");
  const runBrainContent = fileExists(runBrainPath) ? readFile(runBrainPath) : null;
  const buildSpecPath   = path.join(runDir, "handoff", "build-spec.md");
  const buildSpec       = fileExists(buildSpecPath) ? readFile(buildSpecPath).slice(0, 4000) : "";

  const failureList = failures.map((f) =>
    `[FAIL] Criterion ${f.criterionId}: ${f.description}` +
    (f.expected ? `\n  Expected: ${f.expected}` : "") +
    (f.observed ? `\n  Observed: ${f.observed}` : "")
  ).join("\n\n");

  const systemPrompt = buildSystemPrompt(
    `You are the orchestrator for Darkroom. Verification has failed on some criteria. Decide whether to fix the failures or accept them and proceed.

Respond with valid JSON only:
- To fix:    {"decision":"fix","confidence":"high"|"medium"|"low","feedback":"...specific, actionable instructions for the fix agent...","reasoning":"..."}
- To accept: {"decision":"accept","confidence":"high"|"medium"|"low","reasoning":"...why these failures are acceptable (e.g. genuine spec conflict, cosmetic, out of scope)..."}
- To escalate to human: {"decision":"escalate","reasoning":"...why you cannot decide confidently..."}

Use "accept" when a failure is caused by a genuine conflict within the spec itself, is cosmetic and non-functional, or is clearly out of scope for this build. Use "fix" when the code has a real bug that can be corrected.
Your fix feedback should clearly describe what needs to be corrected. Be concrete and direct.`,
    `## Global Brain\n\n${brainContent}`,
    runBrainContent ? `## Run Brain\n\n${runBrainContent}` : null,
    buildSpec ? `## Build Spec (excerpt)\n\n${buildSpec}` : null
  );

  const result = claudeCall(
    systemPrompt,
    `## Decision Point: Verification Feedback\n\nVerification failed on ${failures.length} criterion${failures.length === 1 ? "" : "a"}. Generate feedback for the fix agent.\n\n${failureList}`,
    (u) => logTokens("Verification Feedback Decision", u)
  );

  if (result.decision === "escalate" || result.confidence === "low") {
    writeDecision(runDir, { decisionPoint: "verification-feedback", evidence: failureList.slice(0, 500), brainContext: "(see brain.md)", decision: "escalate", reasoning: result.reasoning ?? "", humanOverride: false });
    console.log(`\n  ${A.yellow("⚑")}  Brain escalating verification feedback: ${result.reasoning ?? "uncertain"}\n`);
    const feedback = await io.turn('Your feedback (or "accept" to proceed with failures): ');
    if (/^(skip|accept|proceed)$/i.test(feedback.trim())) {
      writeDecision(runDir, { decisionPoint: "verification-feedback", evidence: failureList.slice(0, 200), brainContext: "(escalated → human accepted)", decision: "accept", reasoning: "Human accepted failures after escalation.", humanOverride: true });
      return null;
    }
    if (!feedback.trim()) { console.log("No feedback provided. Aborting."); process.exit(1); }
    writeDecision(runDir, { decisionPoint: "verification-feedback", evidence: failureList.slice(0, 200), brainContext: "(escalated)", decision: "fix", reasoning: "Human override after escalation.", humanOverride: true });
    return feedback;
  }
  ...
  if (result.decision === "accept") {
    writeDecision(runDir, { decisionPoint: "verification-feedback", evidence: failureList.slice(0, 500), brainContext: "(verification failures → accept)", decision: "accept", reasoning: result.reasoning ?? "", humanOverride: false });
    console.log(`\n  ${A.cyan("●")}  Auto decision: verification-feedback → accept  ${A.dim("(" + confidenceColor(result.confidence ?? "?") + " confidence)")}`);
    if (result.reasoning) console.log(`  ${A.dim("↳ " + result.reasoning.split(/[.\n]/)[0].trim())}\n`);
    return null;
  }

  writeDecision(runDir, { decisionPoint: "verification-feedback", evidence: failureList.slice(0, 500), brainContext: "(verification failures → fix)", decision: "fix", reasoning: result.reasoning ?? "", humanOverride: false });
  console.log(`\n  ${A.cyan("●")}  Auto decision: verification-feedback → fix  ${A.dim("(" + confidenceColor(result.confidence ?? "?") + " confidence)")}`);
  if (result.reasoning) console.log(`  ${A.dim("↳ " + result.reasoning.split(/[.\n]/)[0].trim())}\n`);
  return result.feedback ?? `Fix the ${failures.length} failing criterion${failures.length === 1 ? "" : "a"} listed above.`;
}
...
async function handleBuildSignal(runDir, signal, io, logTokens, managerRoleId, orgProfile) {
  ...
  if (signal.point === "architect-escalate") {
    // Informational only — the runner locks the plan itself after emitting this and never reads stdin.
    console.log(`\n  ${A.yellow("⚑")}  Architect escalation flagged: ${signal.reason ?? "requires human review"}\n`);
    return "";
  }
...
async function runGraph(profile, opts) {
  ...
  while (currentId) {
    ...
    if (skip) {
      ...
      const nodeArgs = node.manager ? [...makeArgs(), "--manager", node.manager] : makeArgs();
      ...
      if (node.feedbackLoop) {
        ...
        result = handler
          ? await runDivisionAuto(node.runner, nodeArgs, handler, runDir)
          : runDivision(node.runner, nodeArgs);
      }
