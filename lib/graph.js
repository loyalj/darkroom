...
/**
 * graph.js — Pipeline graph executor for the Software Factory.
 *
...
async function runBuildWithFeedback(runId, runDir, mode, io, logTokens) {
  ...
  while (true) {
    const exitCode = mode === "auto"
      ? await runDivisionAuto("departments/run-build.js", ["--run-id", runId], (sig) => handleBuildSignal(runDir, sig, io, logTokens), runDir)
...
async function makeCopyReviewDecision(runDir, io, logTokens) {
  const copyReviewPath = path.join(runDir, "build", "copy-review.txt");
  const copyContent    = readFile(copyReviewPath);
  const brainContent   = org.getBrainForDecision("copy-review");
...
async function makeSecurityFindingDecision(runDir, signal, io, logTokens) {
  const finding        = signal.finding ?? "(finding not available)";
  const brainContent   = org.getBrainForDecision("security-finding");
  const runBrainPath   = path.join(runDir, "run-brain.md");
  const runBrainContent = fileExists(runBrainPath) ? readFile(runBrainPath) : null;
...
async function makeSecurityFinalApprovalDecision(runDir, io, logTokens) {
  const brainContent    = org.getBrainForDecision("security-final-approval");
  const runBrainPath    = path.join(runDir, "run-brain.md");
  const runBrainContent = fileExists(runBrainPath) ? readFile(runBrainPath) : null;

  const systemPrompt = buildSystemPrompt(
    `You are the orchestrator for a Software Factory. All security findings have been reviewed. Decide whether to give final approval.
...
async function makeReviewVerdictNoShipDecision(runDir, io, logTokens) {
  ...
  const brainContent    = org.getBrainForDecision("review-verdict-no-ship");
  const runBrainPath    = path.join(runDir, "run-brain.md");
  const runBrainContent = fileExists(runBrainPath) ? readFile(runBrainPath) : null;
...
async function makeReviewVerdictShipDecision(runDir, io, logTokens) {
  const brainContent    = org.getBrainForDecision("review-verdict-ship");
  const runBrainPath    = path.join(runDir, "run-brain.md");
  const runBrainContent = fileExists(runBrainPath) ? readFile(runBrainPath) : null;
  const verdictPath     = path.join(runDir, "review", "verdict-report.md");
  const verdictContent  = fileExists(verdictPath) ? readFile(verdictPath) : "(verdict not available)";
...
async function handleBuildSignal(runDir, signal, io, logTokens) {
  if (signal.point === "copy-review") return makeCopyReviewDecision(runDir, io, logTokens);
  return handleUnknownSignal(runDir, signal, io);
}

async function handleReviewSignal(runDir, signal, io, logTokens) {
  if (signal.point === "review-verdict-no-ship") return makeReviewVerdictNoShipDecision(runDir, io, logTokens);
  if (signal.point === "review-verdict-ship")    return makeReviewVerdictShipDecision(runDir, io, logTokens);
  return handleUnknownSignal(runDir, signal, io);
}

async function handleSecuritySignal(runDir, signal, io, logTokens) {
  if (signal.point === "security-block")          return "";
  if (signal.point === "security-finding")        return makeSecurityFindingDecision(runDir, signal, io, logTokens);
  if (signal.point === "security-final-approval") return makeSecurityFinalApprovalDecision(runDir, io, logTokens);
  return handleUnknownSignal(runDir, signal, io);
}
...
function getSignalHandler(nodeId, runDir, io, logTokens) {
  if (nodeId === "build")    return (sig) => handleBuildSignal(runDir, sig, io, logTokens);
  if (nodeId === "review")   return (sig) => handleReviewSignal(runDir, sig, io, logTokens);
  if (nodeId === "security") return (sig) => handleSecuritySignal(runDir, sig, io, logTokens);
  return null; // no auto-decision handler — node needs inherited stdio
}
  ...
  while (currentId) {
    ...
    if (skip) {
      ...
      if (node.feedbackLoop) {
        ...
        exitCode = await runBuildWithFeedback(runId, runDir, mode, io, logTokens);
      } else {
        const handler = mode === "auto" ? getSignalHandler(node.id, runDir, io, logTokens) : null;
        exitCode = handler
          ? await runDivisionAuto(node.runner, makeArgs(), handler, runDir)
          : runDivision(node.runner, makeArgs());
      }

      if (exitCode !== 0) abort(runDir, node.id, "Exiting.", n);
