...
/**
 * org.js — Org data layer for Darkroom.
 *
...
function addRoleToProfile(roleId, escalatesTo, profileName) {
...
function addRole(roleData) {
  ...
  addRoleToProfile(role.id, roleData.escalatesTo ?? null, _activeProfileName);
...
// Returns brain content by walking the escalation chain from roleId upward.
// If roleId is null/missing, falls back to the root role of the profile.
// Returns null if no brain is found anywhere in the chain.
function getBrainForRole(roleId, profile) {
  const p = profile ?? getActiveProfile();
  const startId = roleId ?? (p.nodes.find((n) => !n.escalatesTo)?.roleId ?? null);
  if (!startId) return null;
  const chain = getEscalationChain(startId, p);
...
// Legacy: resolve a named decision point to its owning role via decisionRouting,
// then delegate to getBrainForRole. Kept for any callers that haven't migrated.
...
function getBrainForDecision(point, profile) {
  ...
  return getBrainForRole(ownerRole?.id ?? null, p);
}
...
function buildCreateRoleContext(profile) {
  ...
  return [
    ...
    "### Decision Points",
    "",
    DECISION_POINTS.map((dp) => `- **${dp.id}**: ${dp.description}`).join("\n"),
    "",
    "Decisions are routed to the manager assigned to each factory department.",
    "The escalation chain (escalatesTo) handles fallback when a role has no brain.",
    "",
