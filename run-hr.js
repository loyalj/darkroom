...
/**
 * run-hr.js — HR orchestrator for the Software Factory.
 *
 ...
 *
 * Usage:
 *   node run-hr.js --role <id>             # interview a specific role
 *   node run-hr.js --role <id> --run-id <id>  # with explicit run directory (GUI mode)
 */
...
function parseArgs() {
  ...
  if (!roleId && !createRole) {
    console.error("Usage: node run-hr.js --role <id>  |  node run-hr.js --create-role");
    process.exit(1);
  }

  return { roleId, createRole, runId, io };
}
...
async function runInterview(role, runDir, io) {
  ...
  const interviewerPrompt  = readFile(path.join(AGENTS_DIR, "hr", "brain-interviewer.md"));
...
async function runCreateRole(runDir, io) {
  const designerPrompt = readFile(path.join(AGENTS_DIR, "hr", "role-designer.md"));
...
async function main() {
  const { roleId, createRole, runId, io: ioFlag } = parseArgs();
